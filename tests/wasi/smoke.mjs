...
try {
    ...
    const linkedHeader = ok(run(['--paging=never', '--color=never', '--decorations=always', '--style=header,header-path', '--osc8', '/space ü.rs']));
    assert.match(linkedHeader, /Path: \/space ü\.rs/);
    assert.match(linkedHeader, /file:\/\/\/space%20%C3%BC\.rs/);
    ok(run([...plain, '-l', 'c', '-W3'], { stdin: 'int global;\nint value() {\n return 1;\n}\n' }), 'int value() {\n return 1;\n}\n');
