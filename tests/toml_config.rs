...
#[test]
fn local_config_overrides_toml_before_environment_options() {
    let directory = configuration("tabs = 2\n");
    let project = tempfile::tempdir().unwrap();
    fs::write(project.path().join(".batconfig"), "--tabs=5\n").unwrap();
    for (environment, expected) in [(None, "5\n"), (Some("7"), "7\n")] {
        let mut command = bat_with_config();
        command
            .current_dir(project.path())
            .env("BAT_CONFIG_DIR", directory.path())
            .args(["--local-config", "--show-config", "tabs"]);
