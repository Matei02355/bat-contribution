...
#[test]
fn automatic_rebuild_imports_changed_json_theme_colors() {
    let assets = Assets::new();
    let themes = assets.source.path().join("themes");
    fs::create_dir(&themes).unwrap();
    let path = themes.join("automatic-json.json");
    let theme = |color: &str| {
        format!(
            r##"{{"name":"Automatic JSON","colors":{{"editor.foreground":"{color}","editor.background":"#010101"}},"tokenColors":[]}}"##
        )
    };
    fs::write(&path, theme("#123456")).unwrap();
    assets.build(true);
    for (color, escape) in [("#123456", "38;2;18;52;86"), ("#654321", "38;2;101;67;33")] {
        fs::write(&path, theme(color)).unwrap();
        bat()
            .env("BAT_CACHE_PATH", assets.cache.path())
